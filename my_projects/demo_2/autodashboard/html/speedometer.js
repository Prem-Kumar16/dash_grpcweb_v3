...
//   let y = window.matchMedia("(min-width: 965px)");
//   if(x.matches) {
//     setDimensions(x); // Call listener function at run time
//   }
// //   x.addListener(setDimensions); // Attach listener function on state changes

if (screen.width >= 960) {// && screen.orientation.type.includes("landscape")) {
    canvas.width = "800";
    canvas.height = "400";
} else if(screen.width >= 768) {// && screen.orientation.type.includes("landscape")) {
    canvas.width = "765";
    canvas.height = "380";
} else if(screen.width >= 650) {// && screen.orientation.type.includes("landscape")) {
    canvas.width = "600";
    canvas.height = "350";
    mobileLandscape = true;
}

let radius, mainCircleRadiusCalcValue, lcircleOffset1, lcircleOffset2, lcircleDrawMultiplierVal, rcircleOffset1, rcircleOffset2, mCircleOffset;
if(mobileLandscape) {
    radius = 0.293;
    mainCircleRadiusCalcValue = 2.2;
    lcircleOffset1 = 8;
    lcircleOffset2 = 8;
    rcircleOffset1 = 8;
    rcircleOffset2 = 8;
    lcircleDrawMultiplierVal = -35;
    mCircleOffset = 32;
} else {
    radius = 0.35;
    mainCircleRadiusCalcValue = 2;
    lcircleOffset1 = 10;
    lcircleOffset2 = 10;
    rcircleOffset1 = 10;
    rcircleOffset2 = 10;
    lcircleDrawMultiplierVal = -50;
    mCircleOffset = 32.5;
}
...
class Circle {
    ...
    constructor(options) {
        ...
        this.radius = canvas.height * radius;
    }
...
class MainCircle extends Circle {
    ...
    constructor(options) {
        ...
        this.radius = (ch / mainCircleRadiusCalcValue +
            (ch / mainCircleRadiusCalcValue - ch / mainCircleRadiusCalcValue * Math.sin(deg2Rad(50))) / 2 -
...
function drawLeftCircle(options, radius, value, iconStates) {
    ...
    lCircle.drawMultiplier(1, lcircleDrawMultiplierVal);
...
function draw(speedometerValue, tachometerValue, gasValue, mileage, turnSignals, iconStates) {
    ...
    mCircle.drawTextMarks(
        angles=textMarksAngles,
        step=20,
        offset=mCircleOffset
    );

    if(mobileLandscape) {
        mCircle.drawTurnSignal(35, -45, 50, 30, turnSignals['right']);
        mCircle.drawTurnSignal(-35, -45, -50, 30, turnSignals['left']);
    } else {
        mCircle.drawTurnSignal(55, -55, 60, 35, turnSignals['right']);
        mCircle.drawTurnSignal(-55, -55, -60, 35, turnSignals['left']);
    }
    ...
    if(mobileLandscape) {
        mCircle.drawMileage(mileage, 0, 88, 170, 35, 2);
        mCircle.drawUnit(-50);
    } else {
        mCircle.drawMileage(mileage, 0, 118, 205, 35, 2);
        mCircle.drawUnit(-65);
    }
...
if(mobileLandscape) {
    icons = {
        // main circle
        'dippedBeam': new Icon(-30, 67.5, 10, 150 * 0 + 10 * 1, 28, 28, 2),
        'brake':      new Icon(-122, 67.5, 10, 150 * 1 + 10 * 2, 28, 28, 2),
        'drift':      new Icon(7, 67.5, 10, 150 * 2 + 10 * 3, 28, 28, 2),
        'highBeam':   new Icon(-70, 67.5, 10, 150 * 4 + 10 * 5, 28, 28, 2),
        'lock':       new Icon(120, 67.5, 10, 150 * 6 + 10 * 7, 28, 28, 2),
        'seatBelt':   new Icon(75, 67.5, 10, 150 * 8 + 10 * 9, 28, 28, 2),
        'engineTemp': new Icon(103, 108, 10, 150 * 10 + 10 * 11, 28, 28, 3),
        'stab':       new Icon(40, 67.5, 10, 150 * 12 + 10 * 13, 28, 28, 2),
        'abs':        new Icon(-105, 108, 10, 150 * 14 + 10 * 15, 28, 28, 2),
        // right circle
        'gas':        new Icon(10, 80, 10, 150 * 3 + 10 * 4, 25, 25, 3),
        'trunk':      new Icon(42, 45, 10, 150 * 7 + 10 * 8, 30, 30, 2),
        'bonnet':     new Icon(-6, 45, 10, 150 * 11 + 10 * 12, 30, 30, 2),
        'doors':      new Icon(-20, 73, 10, 150 * 15 + 10 * 16, 28, 28, 2),
        // left circle
        'battery':    new Icon(17, 70, 10, 150 * 5 + 10 * 6, 25, 25, 3),
        'oil':        new Icon(0, 45.5, 10, 150 * 9 + 10 * 10, 30, 30, 3),
        'engineFail': new Icon(-25, 70, 10, 150 * 13 + 10 * 14, 28, 28, 3)
    }
} else {
    icons = {
        // main circle
        'dippedBeam': new Icon(-40, 97.5, 10, 150 * 0 + 10 * 1, 35, 35, 2),
        'brake':      new Icon(-150, 97.5, 10, 150 * 1 + 10 * 2, 35, 35, 2),
        'drift':      new Icon(7, 97.5, 10, 150 * 2 + 10 * 3, 35, 35, 2),
        'highBeam':   new Icon(-90, 97.5, 10, 150 * 4 + 10 * 5, 35, 35, 2),
        'lock':       new Icon(150, 97.5, 10, 150 * 6 + 10 * 7, 35, 35, 2),
        'seatBelt':   new Icon(95, 97.5, 10, 150 * 8 + 10 * 9, 35, 35, 2),
        'engineTemp': new Icon(122, 135, 10, 150 * 10 + 10 * 11, 35, 35, 3),
        'stab':       new Icon(50, 97.5, 10, 150 * 12 + 10 * 13, 35, 35, 2),
        'abs':        new Icon(-122, 138, 10, 150 * 14 + 10 * 15, 35, 35, 2),
        // right circle
        'gas':        new Icon(15, 100, 10, 150 * 3 + 10 * 4, 30, 30, 3),
        'trunk':      new Icon(45, 45, 10, 150 * 7 + 10 * 8, 40, 40, 2),
        'bonnet':     new Icon(-15, 45, 10, 150 * 11 + 10 * 12, 40, 40, 2),
        'doors':      new Icon(-30, 85, 10, 150 * 15 + 10 * 16, 35, 35, 2),
        // left circle
        'battery':    new Icon(27.5, 90, 10, 150 * 5 + 10 * 6, 35, 35, 3),
        'oil':        new Icon(5, 52.5, 10, 150 * 9 + 10 * 10, 40, 40, 3),
        'engineFail': new Icon(-25, 90, 10, 150 * 13 + 10 * 14, 35, 35, 3)
    }    
}
...
if(mobileLandscape) {
    options.circleBorderWidth = 6;
    options.markFontSize = 19;
    options.arrowBaseWidth = 5;
    options.arrowBorderWidth = 3.8;
}
